...
/**
 * Swift Mailer file sending plugin
...
/**
 * Swift file send plugin
 * Sends the message using file when a SendEvent is fired.  Using the NativeMail connection provides stub responses to allow this to happen cleanly.
 * @package Swift_Connection
 * @author Chris Corbyn <chris@w3style.co.uk>
 */
class sfEmail_Plugin_File implements Swift_Events_SendListener, Swift_Events_BeforeSendListener
{
  /**
  ...
  /**
   * Constructor.
   * @param string 5th file function parameter as a sprintf() formatted string where %s is the sender.
   */
  ...
  /**
   * Set the 5th file function parameter as a sprintf() formatted string where %s is the sender.
   * @param string
   */
  ...
  /**
   * Get the 5th file function parameter as a sprintf() string.
   * @return string
   */
   ...
   * @param Swift_Events_SendEvent The event information
   * @throws Swift_ConnectionException If file returns false
   */
...
//    else $success = mail($to, $subject, $message, $headers);
    emailPluginWriteMail($message, $subject, "To: {$to}\n" . $headers);
    
//    if (!$success)
//    {
//      @ini_set("sendmail_from", $original_from);
//      throw new Swift_ConnectionException("Sending failed using file as PHP's default file function returned boolean FALSE.");
//    }
    @ini_set("sendmail_from", $original_from);
  }
}
