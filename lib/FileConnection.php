...
/**
 * sfEmailPlugin file connection component
...
/**
 * sfEmailPlugin file Connection
 * NOTE: This class is nothing more than a stub.  The MailSend plugin does the actual sending.
 * @package Swift_Connection
 * @author Chris Corbyn <chris@w3style.co.uk>
 * @author Ton Sharp <forma@66ton99.org.ua>
 */
class sfEmail_Connection_File extends Swift_ConnectionBase
{
  /**
  ...
  /**
   * The 5th parameter in file is a sprintf() formatted string.
   * @var string
   */
  ...
  /**
   * Ctor.
   * @param string The 5th parameter in file as a sprintf() formatted string where %s is the sender address. This only comes into effect if safe_mode is OFF.
   */
  ...
  {
    $this->plugin = new sfEmail_Plugin_File($this->getAdditionalMailParams());
    $instance->attachPlugin($this->plugin, "_FILE_SEND");
  }
  /**
   * Set the 5th parameter in file as a sprintf() formatted string. Only used if safe_mode is off.
   * @param string
   */
  ...
  /**
   * Get the 5th parameter in file as a sprintf() formatted string.
   * @return string
   */
